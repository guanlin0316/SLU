    ...
    def train(self):
        ...
        for epoch in range(0, self.__dataset.num_epoch):
            ...
            dev_slot_f1_score, dev_intent_f1_score, dev_intent_acc_score, dev_sent_acc_score = self.estimate(
                ...
                args=self.args)
            with open(self.args.log_dir + 'logs.txt', "a", encoding="utf-8") as f:
                f.write(
                    "\nDev result: epoch: {}, slot f1 score: {:.6f}, intent f1 score: {:.6f}, intent acc score:"
                    "{:.6f}, semantic accuracy score: {:.6f}.".format(epoch, dev_slot_f1_score, dev_intent_f1_score,
                                                                      dev_intent_acc_score, dev_sent_acc_score))
            ...
            if dev_sent_acc_score > best_dev_sent:
                ...
                with open(self.args.log_dir + 'logs.txt', "a", encoding="utf-8") as f:
                    f.write(
                        "\nTest result: epoch: {}, slot f1 score: {:.6f}, intent f1 score: {:.6f}, intent acc score:"
                        "{:.6f}, semantic accuracy score: {:.6f}.".format(epoch, test_slot_f1, test_intent_f1,
                                                                          test_intent_acc, test_sent_acc))
